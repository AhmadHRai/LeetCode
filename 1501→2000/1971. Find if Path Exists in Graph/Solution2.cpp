...
public:
    bool validPath(int n, vector<vector<int>>& edges, int source, int destination) {
        ...
        // Initialize a stack for DFS and mark the source node as visited
        stack<int> s;
        s.push(source);
        visited[source] = 1; 
        while(!s.empty()) {
...
Approach:
    ...
    This algorithm uses a Depth-First Search (DFS) to determine if there is a path from the source node to the destination node in a graph. The graph is represented as 
...
Complexity Analysis:
    ...
    node and each edge is visited exactly once by the DFS algorithm.
