...
/*
Approach Summary:
    This solution uses a stack to track characters and the KMP algorithm to efficiently match the pattern. It iteratively removes occurrences of the substring by 
    backtracking using the KMP longest prefix-suffix array when mismatches occur.

Complexity Analysis:
    Time Complexity: O(N + M)
        where N is the length of the input string and M is the length of the substring, because each character is visited once and the KMP array is precomputed.
    
    Space Complexity: O(N + M)
        for storing characters in the stack and the KMP array.
*/